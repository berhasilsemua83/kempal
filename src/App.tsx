...
// Paksa pindah ke halaman License jika belum ada lisensi (kecuali sedang buka Info)
  ...
  {!fullView && <Sidebar view="flow" setView={setView} profile={profile} licenseState={licenseState} customServices={customServices} onAddService={() => setAddServiceOpen(true)} onDeleteService={deleteService} licensed={licensed} websiteUrl={appConfig.websiteUrl} />}
  ...
  return (
    <div className="app">
      <Sidebar view={view} setView={setView} profile={profile} licenseState={licenseState} customServices={customServices} onAddService={() => setAddServiceOpen(true)} onDeleteService={deleteService} licensed={licensed} websiteUrl={appConfig.websiteUrl} />
...
{/* --- TAMBAHKAN TOMBOL WEBSITE & KOMUNITAS DI SINI --- */}
      <button
        onClick={() => invoke("open_external_url", { url: websiteUrl })}
      >
...
function SidebarIcon({ name }: { name: "accounts" | "favorites" | "license" | "updates" | "info" | "settings" | "website" }) {
